...
class NavigatorWrappingScreen extends Component {
  render() {
    ...
    let router = SimpleApp.router;//this.props.router;
    ...
    return (
      <View style={{flex: 1}}>
        {routes.map((route, idx) => {
            ...
            //const options = router.getScreenOptions(navigation, {});
    ...
    return (
      <View style={{ flex: 1 }}>
        ...
        <View style={styles.container}>
          {routes.map(route => (
            <TouchableOpacity key={route.routeName} onPress={() => navigation.navigate(route.routeName)}
            >
